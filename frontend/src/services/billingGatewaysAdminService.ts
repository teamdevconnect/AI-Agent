...
export const billingGatewaysAdminService = {
  async list(): Promise<GatewayConfigStatus[]> {
    const { data } = await axiosClient.get<GatewayConfigStatus[]>('/billing/admin/gateways');
    return data;
  },

  async upsert(provider: GatewayProvider, mode: GatewayMode, credentials: Record<string, string>): Promise<GatewayConfigStatus> {
    const { data } = await axiosClient.post<GatewayConfigStatus>('/billing/admin/gateways', { provider, mode, credentials });
    return data;
  },

  async setActive(provider: GatewayProvider, mode: GatewayMode, active: boolean): Promise<GatewayConfigStatus> {
    const { data } = await axiosClient.post<GatewayConfigStatus>(`/billing/admin/gateways/${provider}/${mode}/${active ? 'activate' : 'deactivate'}`);
    return data;
  },
};
