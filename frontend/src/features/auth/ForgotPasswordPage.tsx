...
import { authService, MOCK_OTP } from '@/services/authService';
...
export function ForgotPasswordPage() {
  ...
  const onSubmitEmail = async (values: ForgotPasswordFormValues) => {
    try {
      ...
      setStep('reset');
      toast.success(`Verification code sent (demo code: ${MOCK_OTP})`);
    } catch (error) {
    ...
    return (
      <form className={styles.form} onSubmit={resetForm.handleSubmit(onSubmitReset)} noValidate>
        <div className={styles.hintBox}>
          Enter the code sent to {maskedEmail}. Demo code: {MOCK_OTP}
        </div>
        <Input
